...
begin
  ...
  -- Anchor the interview deadline to this candidate's own registration instant
  -- (now(), the same moment as the insert above) plus their allotted interview
  -- duration, capped at the assessment campaign's overall due_at if that is sooner.
  expires_at := least(
